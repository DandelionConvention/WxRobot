...
def decode_dat(file_path, save_path):
    """
    解密文件，并生成图片
    :param file_path: dat文件路径
    :return: 无
...
def dat2jpg(abs_path: str, save_path: str):
    if not os.path.exists(abs_path) or '.dat' not in abs_path:
        return False

    if '.jpg' not in save_path:
        return False

    decode_dat(abs_path, save_path)
